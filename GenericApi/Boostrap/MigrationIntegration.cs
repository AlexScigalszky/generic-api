...
namespace GenericAPI.Boostrap
{
    public static class SwaggerIntegrationExtension
    {
        ...
        public static void UseSwaggerGen(this IApplicationBuilder applicationBuilder, IWebHostEnvironment environment)
        {
            ...
            {
                c.SwaggerEndpoint("../swagger/v1/swagger.json", "GenericApi");
            });

            applicationBuilder.UseWhen(context => context.Request.Path.StartsWithSegments("/errors", StringComparison.OrdinalIgnoreCase), appBuilder =>
            {
                appBuilder.Use(next =>
                {
                    return async ctx =>
